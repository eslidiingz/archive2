<style>
.radio-toolbar {
	width: 100%;
}
.radio-toolbar input[type="radio"] {
  display: none;
}

.radio-toolbar label {
	display: flex;
    padding: 15px 11px;
    font-size: 16px;
    cursor: pointer;
    color: #000;
    background-color: #fafaff;
    border-radius: 5px;
    width: 100%;
    border: 1px solid #fafaff;
    justify-content: center;
    align-items: center;
}

.radio-toolbar label:hover {
	background-color: #f9f9f9;
    border: 1px solid #34c38f;
}

.radio-toolbar input[type="radio"]:checked+label {
    background-color: #f9f9f9;
    border: 1px solid #34c38f;
}
</style>

<div class="row">
	<!-- <div class="col-lg-6 col-md-6 col-12">
		<div class="row mx-0">
			{{-- <div class="col-12 mt-4 ">
				<input class="form-check-input" type="radio" name="exampleRadios" id="exampleRadios11" value="option11" checked>
				<label class="form-check-label" for="exampleRadios11">
					<img src="new_ui/img/bank/bank03.png" class="pr-2" alt="">ธนาคารกรุงเทพ
				</label>
			</div>
			<div class="col-12 mt-4 ">
				<input class="form-check-input" type="radio" name="exampleRadios" id="exampleRadios22" value="option22" >
				<label class="form-check-label" for="exampleRadios22">
					<img src="new_ui/img/bank/bank04.png" class="pr-2" alt="">ธนาคารกรุงไทย
				</label>
			</div> --}}
			<div class="col-12 mt-4 mx-3">
				<input class="form-check-input" type="radio" name="exampleRadios" id="bank1" value="bank1" checked>
				<label class="form-check-label" for="bank1">
					<img src="new_ui/img/bank/bank02.png" class="pr-2" alt="">ธนาคารไทยพาณิชย์
				</label>
			</div>
		</div>
	</div> -->

	<!-- New  -->
		<div class="col-lg-4 col-md-6 col-12">
			<div class="radio-toolbar">
				<input class="form-check-input" type="radio" name="exampleRadios" id="bank1" value="bank1" style="position: inherit;" checked>
				<label class="form-check-label" for="bank1">
					<img src="new_ui/img/bank/bank02.png" class="pr-2" alt="" style="margin-left: 1em;">ธนาคารไทยพาณิชย์
				</label>
			</div>
		</div>
		{{-- <div class="col-lg-4 col-md-6 col-12">
			<div class="radio-toolbar">
				<input class="form-check-input" type="radio" name="exampleRadios" id="exampleRadios11" value="option11">
				<label class="form-check-label" for="exampleRadios11">
					<img src="new_ui/img/bank/bank03.png" class="pr-2" alt="">ธนาคารกรุงเทพ
				</label>
			</div>
		</div>
		<div class="col-lg-4 col-md-6 col-12">
			<div class="radio-toolbar">
				<input class="form-check-input" type="radio" name="exampleRadios" id="exampleRadios22" value="option22">
				<label class="form-check-label" for="exampleRadios22">
					<img src="new_ui/img/bank/bank04.png" class="pr-2" alt="">ธนาคารกรุงไทย
				</label>
			</div>
		</div> --}}
	<!-- New  -->

	<!-- <div class="col-lg-6 col-md-6 col-12">
		<div class="row mx-md-0 mx-lg-0 mx-xl-0"> -->
			<!-- <div class="col-12 mt-4">
				<h6 class="mb-0"><strong>ชำระเงินอย่างไร</strong></h6>
			</div>
			<div class="col-12">
				<hr class="w-100">
			</div>
			<div class="col-12">
				<p>
					1. เลือกจ่ายบิล / ชำระค่าบริการ <br>
					2. เลือกอื่นๆ > รหัสบริษัท > ระบุรหัสบริษัท > ออมทรัพย์<br>
					3. ใส่รหัสบริษัท 12345<br>
					4. ระบุหมายเลขอ้างอิงการชำระเงินสำหรับ “REF1”<br>
					5. ระบุหมายเลขโทรศัพท์มือถือ สำหรับ “REF2”<br>
					6. ระบุยอดเงิน<br>
					7. หมายเหตุ : ค่าธรรมเนียมขึ้นกับธนาคา หรือ ผู้ให้บริการ
				</p>
			</div> -->
		<!-- </div>
	</div> -->
</div>
