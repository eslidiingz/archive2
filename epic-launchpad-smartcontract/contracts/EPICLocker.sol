...
contract EPICLocker is Pausable, AccessControl, ReentrancyGuard {
    ...
    function unsetSaleType(string memory _sale, bool _exist) public whenNotPaused onlyRole(DEFAULT_ADMIN_ROLE) {
    ...
    function unlock(
        ...
        for (uint256 index = 0; index < _expirationList.length; index++) {
            if (_expiration == _expirationList[index]) {
                ...
                epicToken.safeTransfer(msg.sender, _amount);
